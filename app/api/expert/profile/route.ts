...
async function verifyAuth() {
  ...
  try {
    const decoded = jwt.verify(token.value as string, JWT_SECRET as string) as {
      userId: string;
    };
...
export async function GET() {
  try {
    ...
    const bookings = expert.bookings;

    // Calculate stats
    const completedBookings = bookings.filter(
      (b) => b.status === "completed" || b.call
    );
    const completedCalls = completedBookings.length;

    const totalEarnings = completedBookings.reduce((acc, b) => {
      return acc + (b.call?.amountCharged || b.cost || 0);
    }, 0);

    const callsWithRatings = bookings.filter((b) => b.call?.rating);
    const totalReviews = callsWithRatings.length;
    const ratingSum = callsWithRatings.reduce(
      (acc, b) => acc + (b.call?.rating || 0),
      0
    );
    const averageRating =
