...
import { Briefcase, Search, MoreHorizontal, Check, X, Eye, Trash, Ban, Star } from "lucide-react"
...
export default function AdminExpertsPage() {
  ...
  const handleAction = (expert: Expert, action: "approve" | "reject" | "suspend" | "delete") => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "approved":
        return <Badge className="bg-chart-3 text-chart-3-foreground">Approved</Badge>
      case "pending":
        return (
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold">Expert Management</h1>
              <p className="text-muted-foreground">Review and manage expert accounts</p>
            </div>
          </div>
          ...
          <Card className="glass border-border/50">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-chart-3">
                {mockExperts.filter((e) => e.status === "approved").length}
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="glass border-border/50">
          <CardHeader>
            ...
            <CardDescription>Manage expert applications and accounts</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                ...
                {filteredExperts.map((expert) => (
                  <TableRow key={expert.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar>
                          <AvatarImage src={expert.user.avatar || "/placeholder.svg"} />
                          <AvatarFallback>
                            {expert.user.name
                              .split(" ")
                              .map((n) => n[0])
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{expert.user.email}</p>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>{expert.completedCalls}</TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          {expert.status === "pending" && (
                            <>
                              <DropdownMenuItem onClick={() => handleAction(expert, "approve")}>
                                <Check className="w-4 h-4 mr-2 text-chart-3" />
                                Approve
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => handleAction(expert, "reject")}>
                          ...
                          {expert.status === "approved" && (
                            <DropdownMenuItem onClick={() => handleAction(expert, "suspend")}>
                              <Ban className="w-4 h-4 mr-2 text-yellow-500" />
                              Suspend
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem onClick={() => handleAction(expert, "delete")} className="text-destructive">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <Avatar className="w-16 h-16">
                    <AvatarImage src={selectedExpert.user.avatar || "/placeholder.svg"} />
                  ...
                  <div>
                    <h3 className="font-semibold text-lg">{selectedExpert.user.name}</h3>
                    <p className="text-sm text-muted-foreground">{selectedExpert.user.email}</p>
                ...
                <div className="grid grid-cols-2 gap-4 p-4 rounded-lg bg-muted/20">
                  ...
                  <div>
                    ...
                    <p className="font-medium">₦{selectedExpert.ratePerMin}/min</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Rating</p>
                    <p className="font-medium">
                      {selectedExpert.rating} ({selectedExpert.totalReviews} reviews)
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Completed Calls</p>
                    <p className="font-medium">{selectedExpert.completedCalls}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Total Earnings</p>
                    <p className="font-medium">₦{selectedExpert.totalEarnings.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Wallet</p>
                    <p className="font-mono text-sm">{selectedExpert.user.walletAddress}</p>
                  </div>
                </div>
                <div>
        ...
        <Dialog open={actionModalOpen} onOpenChange={setActionModalOpen}>
          <DialogContent className="glass border-border/50">
            <DialogHeader>
              <DialogTitle className="capitalize">{actionType} Expert</DialogTitle>
              <DialogDescription>
                Are you sure you want to {actionType} {selectedExpert?.user.name}?
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
